        ...
        while self.is_alive:
            ...
            if arg == 'show':
                ...
                try:
                    ...
                    # Defensive: do not exit the loop on an unexpected KeyError.
                    # Continue processing further commands.
                    continue
                except JavascriptException as e:
                    # Safely parse the exception payload without assuming 'line'/'column' keys.
                    # Use ast.literal_eval to avoid executing arbitrary code; fall back to string message.
                    try:
                    ...
                    # If we have a dict-like payload, extract fields safely.
                    if isinstance(msg, dict):
                        name = msg.get("name", "JavascriptException")
                        line = msg.get("line", "?")
                        column = msg.get("column", "?")
                        message = msg.get("message", "")
                        stack = msg.get("stack", "")

                        # Optionally ignore the known lightweight-charts crosshair null error
                        # which happens when moving the mouse over the main chart:
                        # "Value is null" coming from setCrosshairPosition.
                        if "Value is null" in message or "setCrosshairPosition" in stack:
                            # swallow this known, non-fatal error to avoid crashing the loop
                            # (you can log it here instead of ignoring)
                            continue

                        # raise a clearer JavascriptException including available info
                        raise JavascriptException(
                            f"\n\nscript -> '{arg}',\nerror -> {name}[{line}:{column}]\n{message}"
                        )
                    else:
                        # msg is not a dict (string or other). Check text for known crosshair error:
                        text = str(msg)
                        if "Value is null" in text or "setCrosshairPosition" in text:
                            continue
